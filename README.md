# 🚀 Bonjour, je suis Baptiste BERSUDER

**Apprenti Développeur Logiciel & Data Scientist** 
📍 Strasbourg, France 
🎓 Étudiant en Licence Informatique (CNAM) & admis au Mastere MAALSI (CESI)

---

### 🛠️ Stack Technique

![C++](https://img.shields.io/badge/C%2B%2B-00599C?style=for-the-badge&logo=c%2B%2B&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![C#](https://img.shields.io/badge/C%23-239120?style=for-the-badge&logo=c-sharp&logoColor=white)
![PHP](https://img.shields.io/badge/PHP-777BB4?style=for-the-badge&logo=php&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=for-the-badge&logo=postgresql&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux&logoColor=black)

**Langages :** C, C++, Python (Maîtrise), C#, PHP, SQL 

**Frameworks & Outils :** Qt Creator, .NET, CodeIgniter, Apache Guacamole 

**Systèmes & Réseaux :** Linux (Maîtrise), Windows Server, Cisco Certifications 

---

### 🔬 Expériences Marquantes

**CNRS (Apprentissage) :** Optimisation d'un logiciel embarqué en **C++** pour analyseur de polluants. **Gain de temps de +50%** et fiabilisation accrue des données scientifiques.

**CNRS (Stage) :** Déploiement d'une solution d'accès à distance sécurisée via **Apache Guacamole** et scripts de Pentest en **Python**.

---

### 📂 Projets Phares

* **🚗 Gestion Parc Auto :** Logiciel de bureau en **C# .NET** (IHM, SQL Server, POO) pour la gestion complète d'un garage.
* **🐟 Enchères de Marée :** Plateforme web temps réel utilisant l'architecture **MVC**, **PHP (CodeIgniter)** et **MySQL**.
* **📝 To-Do List :** Application de productivité personnelle développée en **C# .NET** focalisée sur l'ergonomie.

---

### 📫 Me contacter

[![Email](https://img.shields.io/badge/Email-D14836?style=flat-square&logo=gmail&logoColor=white)](mailto:bersuderbaptiste@gmail.com)
**Email :** bersuderbaptiste@gmail.com

**Objectif :** Recherche d'une alternance de 2 ans pour le Mastère MAALSI (CESI) dès la rentrée 2026.

---
